...
def preprocess_data(bank_df, bb_df, name_dict):
    ...
    bb_df['Statement Names'] = bb_df['Statement Names'].apply(lambda x: x if isinstance(x, list) else [x])
    bb_df['Statement Names'] = bb_df['Statement Names'].apply(lambda x: [name.upper() for name in x])
