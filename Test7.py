...
def reconcile(bank_df, bb_df):
    ...
    for _, ar in bb_df.iterrows():
        ...
        possible_names = '|'.join(ar['Statement Names'])
        counterparty_transactions = bank_df[bank_df['DESCRIPTION'].str.contains(possible_names, case=False, na=False)]
        ...
        if counterparty_transactions.empty:
            ...
            match = counterparty_transactions[counterparty_transactions['AMOUNT'].between(ar['Net Billed'] - 1, ar['Net Billed'] + 1)]
            if not match.empty:
                row = match.iloc[0]
